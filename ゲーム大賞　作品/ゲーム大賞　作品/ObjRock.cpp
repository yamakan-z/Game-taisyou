...
void CObjRock::Init()
{
	...
	//マップデータをコピー
	memcpy(m_mapR, rock_data, sizeof(int) * (10 * 100));
}
	...
	for (int i = 0; i < 10; i++)
	{
		for (int j = 0; j < 100; j++)
		{
			...
			if (m_mapR[i][j] > 0)
			{
				...
				//障害物
				if (m_mapR[i][j] == 10 && (hx + (-block->GetScroll()) + 64.0f > x) && (hx + (-block->GetScroll()) < x + 64.0f) && (hy + 64.0f > y) && (hy < y + 64.0f))
				{
					//上下左右判定
					...
					if (len < 88.0f)
					{
						...
						if (r > 45 && r < 135)
						{
							...
							hero->SetBT(m_mapR[i][j]);//ブロックの要素(type)を主人公に渡す
							hero->SetVY(0.0f);
						}
						if (r > 135 && r < 225)
						{
					...
					//障害物判定ブロック
					if (m_mapR[i][j] == 10 && (hx + (-m_scroll) + 64.0f > x) && (hx + (-m_scroll) < x + 64.0f) && (hy + 64.0f > y) && (hy < y + 64.0f))
					{
						//上下左右判定
